...
XRAY_LOG_FILE = CONFIG_DIR / "xray.log"
...
def harden_existing_secret_files() -> None:
    ...
    ensure_dirs()
    for path in (SUB_FILE, SERVERS_FILE, XRAY_CONFIG, PID_FILE, XRAY_LOG_FILE, CODEX_ENV_FILE):
        if path.exists():
            chmod_if_possible(path, SECURE_FILE_MODE)


def open_secure_append(path: Path):
    """Open a file for append without briefly creating it world-readable."""
    path.parent.mkdir(parents=True, exist_ok=True)
    fd = os.open(path, os.O_WRONLY | os.O_CREAT | os.O_APPEND, SECURE_FILE_MODE)
    chmod_if_possible(path, SECURE_FILE_MODE)
...
def allow_insecure_subscription_tls(args: list[str] | None = None) -> bool:
    ...
    args = args or []
    return ALLOW_INSECURE_SUB_FLAG in args or truthy(os.environ.get("CODEX_PROXY_ALLOW_INSECURE_SUB", "0"))
...
def save_servers(servers: list[dict[str, Any]]) -> None:
    ensure_dirs()
    secure_write_text(SERVERS_FILE, json.dumps(servers, ensure_ascii=False, indent=2) + "\n")
        ...
        stream["tcpSettings"] = {
            "header": {
                ...
                "request": {
                    ...
                    "headers": {"Host": [server.get("host") or server.get("sni") or ""]},
                },
            }
        }
...
def start_xray(server: dict[str, Any]) -> None:
    ...
    secure_write_text(XRAY_CONFIG, json.dumps(config, ensure_ascii=False, indent=2) + "\n")
    ...
    log_handle = open_secure_append(XRAY_LOG_FILE)
    try:
    ...
    if proc.poll() is not None:
        log_tail = read_text_tail(XRAY_LOG_FILE)
        die(f"\nxray failed to start. Log tail:\n{log_tail}")

    secure_write_text(PID_FILE, str(proc.pid) + "\n")
    print(f"\nxray started (PID {proc.pid})")
    print(f"HTTP proxy: {LOCAL_HTTP_URL}")
    print(f"xray log: {XRAY_LOG_FILE}")
    print("\nTest it:")
    print(f"  curl -x {LOCAL_HTTP_URL} -s -o /dev/null -w 'HTTP Status: %{{http_code}}\\n' https://api.openai.com/v1/models")
    print("Expected: HTTP 401 without API key. 401 is OK here: it means proxy reached OpenAI.")
    print("\nFor Codex CLI:")
...
def open_via_proxy(url: str, timeout: int = 8) -> tuple[bool, str]:
    ...
    proxy = urllib.request.ProxyHandler({"https": LOCAL_HTTP_URL, "http": LOCAL_HTTP_URL})
...
def install_codex_env() -> None:
    ...
    existing = CODEX_ENV_FILE.read_text(encoding="utf-8") if CODEX_ENV_FILE.exists() else ""
    ...
    print("If Codex.app was opened from Finder and ignores .env, use: codex-proxy --run-app")
...
def run_codex_cli(codex_args: list[str]) -> int:
    ...
    if not codex_bin:
        die("codex CLI not found. Install it first: npm install -g @openai/codex or brew install --cask codex")
