...
set -g status-right '#(cd #{q:pane_current_path} && codex-switch status --line)'
```

`codex-switch` must be on the tmux server's PATH. The [tmux format modifier](https://man.openbsd.org/tmux#FORMATS) quotes the pane directory for the shell. This is an example configuration; automated tests cover status output, not tmux rendering.
