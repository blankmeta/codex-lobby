...
class Console:
    def __init__(self, reader=input, writer=print, language: str | None = None, terminal=None):
        self.read, self.write = reader, writer
        self.terminal = terminal
        self.ru = (language or os.environ.get("CODEX_LOBBY_LANG") or os.environ.get("CODEX_SWITCH_LANG") or os.environ.get("LANG", "en")).lower().startswith("ru")
    ...
    def show_accounts(self, accounts: list[Account]) -> None:
        ...
        for i, account in enumerate(accounts, 1):
            ...
            if account.needs_login:
                self.say("       Sign in again: codex-lobby login", "       Нужен повторный вход: codex-lobby login")
            ...
            self.write("")
        self.say("  Limits are snapshots, not a token balance. Refresh: codex-lobby accounts --refresh",
                 "  Лимиты — снимок данных, не баланс токенов. Обновить: codex-lobby accounts --refresh")
    ...
    def show_profiles(self, profiles, bound=None) -> None:
        self.say("\n  Your Codex profiles\n", "\n  Твои профили Codex\n")
        if not profiles:
            self.say("  Add your first profile: codex-lobby login personal", "  Добавь первый профиль: codex-lobby login personal")
            return
        if bound:
        ...
        for index, profile in enumerate(profiles, 1):
            ...
            if account and account.needs_login:
                self.write(f"       codex-lobby login {profile.name}")
        self.say("\n  Refresh: codex-lobby profiles --refresh. Running profiles keep cached data.",
                 "\n  Обновить: codex-lobby profiles --refresh. У работающих профилей данные из кеша.")
