...
class Console:
    def __init__(self, reader=input, writer=print, language: str | None = None, terminal=None):
        ...
        self.ru = (language or os.environ.get("RUNLOBBY_LANG") or os.environ.get("CODEX_LOBBY_LANG") or os.environ.get("CODEX_SWITCH_LANG") or os.environ.get("LANG", "en")).lower().startswith("ru")
    ...
    def show_accounts(self, accounts: list[Account]) -> None:
        ...
        for i, account in enumerate(accounts, 1):
            ...
            self.write("")
        self.say("  Limits are snapshots, not a token balance. Refresh: runlobby accounts --refresh",
                 "  Лимиты — снимок данных, не баланс токенов. Обновить: runlobby accounts --refresh")
    ...
    def show_profiles(self, profiles, bound=None) -> None:
        self.say("\n  Your Codex profiles\n", "\n  Твои профили Codex\n")
        if not profiles:
            self.say("  Add your first profile: runlobby login personal", "  Добавь первый профиль: runlobby login personal")
            return
        if bound:
        ...
        for index, profile in enumerate(profiles, 1):
            ...
            if account and account.needs_login:
                self.write(f"       runlobby login {profile.name}")
        self.say("\n  Refresh: runlobby profiles --refresh. Running profiles keep cached data.",
                 "\n  Обновить: runlobby profiles --refresh. У работающих профилей данные из кеша.")
